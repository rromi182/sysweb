...
final class EmpleadoTable extends PowerGridComponent
{
    ...
    public function setUp(): array
    {
        ...
        return [
            PowerGrid::exportable('empleados_export_' . now()->format('Y_m_d'))
                ->type(Exportable::TYPE_XLS, Exportable::TYPE_CSV),

            PowerGrid::header()
                ->showSearchInput()
                ->includeViewOnTop('livewire.empleados.empleado-table-header'),
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('estado_badge', fn(Empleado $model) => $model->estado_badge)
    ...
    public function actions($row): array
    {
        return [
            Button::add('view')
                ...
                ->class('inline-flex h-7 w-7 items-center justify-center rounded-md text-xs font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring')
