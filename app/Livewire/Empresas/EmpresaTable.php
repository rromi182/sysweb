...
final class EmpresaTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('nombre')
            ->add('razon_social')
            ->add('ruc', fn(Empresa $model) => Str::limit($model->description, 50))
            ->add('direccion')
            ->add('telefono')
            ->add('correo')
            ->add('logo')
    ...
    public function actions(Empresa $row): array
    {
        return [
            ...
            Button::add('edit')
                ...
                ->dispatch('edit-empresa', ['' => $row->id])
                ->tooltip('Edit empresa'),
            ...
            Button::add('delete')
                ...
                ->dispatch('open-delete-modal', [
                    ...
                    'description' => "Are you sure you want to delete empresa '{$row->name}'? This action cannot be undone.",
                ])
                ->tooltip('Delete empresa'),
        ];
    }
    ...
    public function delete($rowId, EmpresaService $service): void
    {
        ...
        if ($empresa) {
            try {
                ...
                $this->dispatch('toast', message: 'Empresa deleted successfully.', type: 'success');
            } catch (\Exception $e) {
                $message = $e instanceof EmpresaException
                    ? $e->getMessage()
                    : 'Failed to delete empresa: ' . $e->getMessage();
